...
module.exports = async (req, res) => {
  ...
  if (req.method === 'DELETE') {
    ...
    if (!supabaseUrl || !serviceKey) {
      sendJson(res, 503, { error: 'Configure the backend SUPABASE_SERVICE_ROLE_KEY to clear orders.' });
      return;
    }
    try {
      const ordersUrl = new URL('/rest/v1/orders', supabaseUrl);
      ordersUrl.searchParams.set('id', 'gt.0');
      const response = await fetch(ordersUrl, {
        method: 'DELETE',
        headers: { apikey: serviceKey, Authorization: `Bearer ${serviceKey}` }
      });
      if (!response.ok) throw new Error(`Order delete returned ${response.status}`);
      sendJson(res, 200, { success: true });
    } catch (error) {
      console.error('Admin order delete failed:', error.message);
      sendJson(res, 502, { error: 'Unable to clear orders.' });
    }
    return;
  }
